...
public sealed class Tests : TestClickHouseStore
{
    ...
    [Fact]
    public async Task WritingRecords_Concurrently()
    {
        var chunks = Enumerable.Range(0, 8)
            .Select(_ => _fixture.CreateMany<FlatObject>(219).ToArray())
            .ToArray();

        await Task.WhenAll(chunks.Select(chunk => Task.Run(() => _writer.Insert(chunk, CancellationToken.None))));

        var written = await GetClickhouseObjects();

        written.Should().BeEquivalentTo(chunks.SelectMany(x => x), o => o.ComparingByValue<FlatObject>());
    }
    ...
    [Fact]
    public async Task WritingToUnknownColumn_Throws()
    {
        var writer = new TableWriterBuilder(this.GetConnectionFactory())
            .For<FlatObject>("test_table")
            .AddColumn("unknown_column", a => a.PartnerId)
            .Build();

        var act = () => writer.Insert(_fixture.CreateMany<FlatObject>(3).ToArray(), CancellationToken.None);
